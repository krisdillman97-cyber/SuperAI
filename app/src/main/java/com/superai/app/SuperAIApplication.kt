...
class SuperAIApplication : Application(), Configuration.Provider {
    ...
    private fun createNotificationChannels() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val channels = listOf(
                NotificationChannel(CHANNEL_HUD,      "Floating HUD",      NotificationManager.IMPORTANCE_LOW).apply {
                    description = "SuperAI floating overlay — persistent"
                    setShowBadge(false)
                },
                NotificationChannel(CHANNEL_COMPILER, "Compiler Engine",   NotificationManager.IMPORTANCE_DEFAULT).apply {
                    description = "On-device build engine progress"
                    setShowBadge(true)
                },
                NotificationChannel(CHANNEL_DRIVE,    "Drive Sync",        NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Google Drive synchronisation"
                    setShowBadge(false)
                },
                NotificationChannel(CHANNEL_AGENT,    "Agent Notifications", NotificationManager.IMPORTANCE_HIGH).apply {
